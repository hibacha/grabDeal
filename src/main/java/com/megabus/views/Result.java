...
public class Result {
  ...
  private final ResultContainer CONTAINER = PageFactory.initElements(
      BrowserDriver.getCurrentDriver(), ResultContainer.class);

  public void isResultShown() {
  ...
  public Map<Itinerary, WebElement> loadTripResultByType(TRIP trip) {
    List<WebElement> result =
        trip.equals(TRIP.DEPARTURE) ? CONTAINER.getDepartureTripResult() : CONTAINER
            .getReturnResult();
  ...
  public Map<Itinerary, WebElement> findBetween(Map<Itinerary, WebElement> itineraryMap,
    ...
    return itineraryMap
        ...
        .stream()
        .filter(
            it -> it.getKey().getTime().compareTo(startTime) >= 0
                && it.getKey().getTime().compareTo(endTime) <= 0)
        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
  }

  public Itinerary findCheapest(Set<Itinerary> itineraries) {
    System.out.println("@@@@" + itineraries.size());
    Itinerary cheapest =
        itineraries.stream().min((it1, it2) -> Float.compare(it1.getFare(), it2.getFare())).get();
    return cheapest;
  }
...
//    }
  ...
  private LocalTime parseLeaveTime(WebElement option) {
  ...
  private float parseFare(WebElement option) {
